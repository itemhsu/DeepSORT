...
void postprocess(cv::Mat & frame,
    const std::vector < cv::Mat > &out, DETECTIONS & d);
...
getOutputsNames(const cv::dnn::Net & net);
...
int main(int argc, char **argv)
{
    ...
    // Load the network
    cv::dnn::Net net =
        cv::dnn::readNetFromDarknet(modelConfiguration,
        modelWeights);
    net.setPreferableBackend(cv::dnn::DNN_BACKEND_OPENCV);
    net.setPreferableTarget(cv::dnn::DNN_TARGET_CPU);

    std::cout << "### main 89: cv::dnn::readNetFromDarknet ###"<< std::endl;
    ...
    try {
	    ...
	    std::cout << "### Capture video 0 120###"<< std::endl;
            cap.open(1);
	    std::cout << "### Capture video 0 122###"<< std::endl;
        }
        // Open the webcaom
    ...
    std::cout << "### main 135: Capture video ###"<< std::endl;
    // Get the video writer initialized to save the output video
    if (!parser.has("image")) {
        video.open(outputFile, cv::VideoWriter::fourcc('M',
                'J', 'P', 'G'), 28.0,
            cv::Size(static_cast <
    ...
    std::cout << "### main 156: while loop ###"<< std::endl;
    ...
    std::cout << "### main 167: while loop 4D blob ###"<< std::endl;
        cv::dnn::blobFromImage(frame, blob, 1 / 255.0,
            cvSize(inpWidth, inpHeight), cv::Scalar(0, 0,
                0), true, false);
        ...
        std::cout << "Detections size:" << detections.
            size() << std::endl;
    std::cout << "### main 185: before FeatureTensor ###"<< std::endl;
    bool tfIns=FeatureTensor::getInstance()->
            getRectsFeature(frame, detections);
	    
    std::cout << "### main 189: before FeatureTensor ###"<< std::endl;
        if (tfIns) {
    std::cout << "### main 188 ###"<< std::endl;
            std::
    ...
    std::cout << "### main 196 ###"<< std::endl;
                if (!track.is_confirmed()
                    || track.time_since_update > 1)
                    continue;
                result.push_back(std::make_pair(track.
                        track_id, track.to_tlwh()));
            }
            for (unsigned int k = 0; k < detections.size();
                k++) {
    std::cout << "### main 205 ###"<< std::endl;
                DETECTBOX tmpbox = detections[k].tlwh;
                cv::Rect rect(tmpbox(0), tmpbox(1),
                    tmpbox(2), tmpbox(3));
                cv::rectangle(frame, rect, cv::Scalar(0, 0,
                        255), 4);
                // cvScalar的储存顺序是B-G-R，CV_RGB的储存顺序是R-G-B

                for (unsigned int k = 0; k < result.size();
                    k++) {
    std::cout << "### main 215 ###"<< std::endl;
                    DETECTBOX tmp = result[k].second;
                    cv::Rect rect =
                        cv::Rect(tmp(0), tmp(1), tmp(2),
                        tmp(3));
                    rectangle(frame, rect, cv::Scalar(255,
                            255, 0), 2);

                    std::string label =
                        cv::format("%d", result[k].first);
    ...
    std::cout << "### main 232 ###"<< std::endl;
            std::
                cout << "Tensorflow get feature failed!" <<
                std::endl;;
        }
    std::cout << "### main 237 ###"<< std::endl;
    ...
    std::cout << "### main 263 ###"<< std::endl;
    cap.release();
    if (!parser.has("image"))
        video.release();

    std::cout << "### main 268 ###"<< std::endl;
    return 0;
}

// Remove the bounding boxes with low confidence using non-maxima suppression
void postprocess(cv::Mat & frame,
    const std::vector < cv::Mat > &outs, DETECTIONS & d)
{
    ...
    cv::dnn::NMSBoxes(boxes, confidences, confThreshold,
    ...
    int right, int bottom, cv::Mat & frame)
{
    ...
    cv::rectangle(frame, cv::Point(left, top),
        cv::Point(right, bottom), cv::Scalar(255, 178, 50),
        3);
