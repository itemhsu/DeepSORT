...
Track::Track(KAL_MEAN & mean, KAL_COVA & covariance,
    int track_id, int n_init, int max_age,
...
void Track::update(KalmanFilter * const kf,
    const DETECTION_ROW & detection)
{
    KAL_DATA pa =
        kf->update(this->mean, this->covariance,
        detection.to_xyah());
    this->mean = pa.first;
