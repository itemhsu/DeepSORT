...
{
    this->metric =
        new NearNeighborDisMetric(NearNeighborDisMetric::
        METRIC_TYPE::cosine, max_cosine_distance,
        nn_budget);
...
void tracker::update(const DETECTIONS & detections)
{
  ...
  for (int &detection_idx:unmatched_detections) {
    ...
    cout << "%%% tracker::update 83 %%%" << endl;
    for (it = tracks.begin(); it != tracks.end();) {
        cout << "%%% tracker::update 84 %%%" << endl;
        if ((*it).is_deleted())
            it = tracks.erase(it);
        else
            ++it;
    ...
    /* old version:
       ...
       track.features = Eigen::Matrix<float, -1, 256, Eigen::RowMajor>(0,256);
       }
       this->metric->partial_fit(features, targets, active_targets);
     */
    vector < int >active_targets;
    vector < TRACKER_DATA > tid_features;
    cout << "%%% tracker::update 113 %%%" << endl;
  for (Track & track:tracks) {
        cout << "%%% tracker::update 115 %%%" << endl;
        if (track.is_confirmed() == false)
            continue;
        cout << "%%% tracker::update 117 %%%" << endl;
        active_targets.push_back(track.track_id);
        cout << "%%% tracker::update 119 %%%" << endl;
        tid_features.push_back(std::make_pair(track.
                track_id, track.features));
        cout << "%%% tracker::update 120 %%%" << endl;
        FEATURESS t = FEATURESS(0, 256);
        cout << "%%% tracker::update 121 %%%" << endl;
        track.features = t;
    }
    cout << "%%% tracker::update 122 %%%" << endl;
    this->metric->partial_fit(tid_features, active_targets);
    cout << "%%% tracker::update 124 %%%" << endl;
}
    ...
    TRACHER_MATCHD & res)
{
  ...
  for (Track & t:tracks) {
    ...
    TRACHER_MATCHD matcha =
        linear_assignment::getInstance()->
        matching_cascade(this, &tracker::gated_matric,
    ...
    TRACHER_MATCHD matchb =
        linear_assignment::getInstance()->
        min_cost_matching(this, &tracker::iou_cost,
    ...
    //get result:
    res.matches.assign(matcha.matches.begin(),
        matcha.matches.end());
    res.matches.insert(res.matches.end(),
        matchb.matches.begin(), matchb.matches.end());
    //unmatched_tracks;
    res.unmatched_tracks.assign(matcha.unmatched_tracks.
        begin(), matcha.unmatched_tracks.end());
    res.unmatched_tracks.insert(res.unmatched_tracks.end(),
        matchb.unmatched_tracks.begin(),
        matchb.unmatched_tracks.end());
    res.unmatched_detections.assign(matchb.
        unmatched_detections.begin(),
        matchb.unmatched_detections.end());
}

void tracker::
_initiate_track(const DETECTION_ROW & detection)
{
    cout << "%%% tracker::_initiate_track 186 %%%" << endl;
    KAL_DATA data = kf->initiate(detection.to_xyah());
    KAL_MEAN mean = data.first;
    KAL_COVA covariance = data.second;

    this->tracks.push_back(Track(mean, covariance,
            this->_next_idx, this->n_init, this->max_age,
            detection.feature));
    _next_idx += 1;
}

DYNAMICM tracker::gated_matric(std::vector < Track >
    &tracks, const DETECTIONS & dets,
    ...
    const std::vector < int >&detection_indices)
{
  ...
  for (int i:track_indices) {
    ...
    DYNAMICM res =
        linear_assignment::getInstance()->
        gate_cost_matrix(this->kf, cost_matrix, tracks,
        dets, track_indices,
        detection_indices);
    return res;
}
    ...
    const std::vector < int >&detection_indices)
{
    ...
    for (int i = 0; i < rows; i++) {
        ...
        if (tracks[track_idx].time_since_update > 1) {
            cost_matrix.row(i) =
                Eigen::RowVectorXf::Constant(cols,
                INFTY_COST);
            continue;
        }
        ...
        for (int k = 0; k < csize; k++)
            candidates.row(k) =
                dets[detection_indices[k]].tlwh;
        Eigen::RowVectorXf rowV =
            (1. - iou(bbox,
                candidates).array()).matrix().transpose();
    ...
    tracker::iou(DETECTBOX & bbox, DETECTBOXSS & candidates)
{
    ...
    for (int i = 0; i < size; i++) {
        float tl_1 =
            std::max(bbox_tl_1, candidates_tl(i, 0));
        float tl_2 =
            std::max(bbox_tl_2, candidates_tl(i, 1));
        float br_1 =
            std::min(bbox_br_1, candidates_br(i, 0));
        float br_2 =
            std::min(bbox_br_2, candidates_br(i, 1));
